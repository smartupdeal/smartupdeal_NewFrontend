  ...
  return async (dispatch, getState) => {
    try {
      ...
      const {
        ...
        totalReviews,
        totalSummary
      } = getReviewsSummary(response.data.reviews);
...
// fetch saleTemp reviews api
export const fetchSaleTempReviews = slug => {
  return async (dispatch, getState) => {
    try {
      const response = await axios.get(`/api/review/saleTemp/${slug}`);
...
export const addSaleTempReview = () => {
  return async (dispatch, getState) => {
    try {
      const rules = {
        title: 'required',
      ...
      const review = getState().review.reviewFormData;
      const saleTemp = getState().saleTemp.saleTemp;

      const newReview = {
        saleTemp: saleTemp._id,
        isRecommended: review.isRecommended.value,
        rating: review.rating,
        review: review.review,
        title: review.title
      ...
      if (response.data.success === true) {
        dispatch(success(successfulOptions));
        dispatch(fetchSaleTempReviews(saleTemp.slug));
